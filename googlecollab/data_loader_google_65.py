...
def smooth_labels_with_dist(labels, smooth_dist):
    ...
    for i in range(smooth_dist.shape[0]):
        assert(np.isclose(np.sum(smooth_dist[i]), 1.0)), "Smooth distribution must sum to 1.0"
...
class DataGenerator(keras.utils.Sequence):
    ...
    def __data_generation(self, indexes):
        ...
        smooth_factor = 0.3
        smooth_dist = np.eye(len(self.classes)) * (1 - smooth_factor) + (1 - np.eye(len(self.classes))) * smooth_factor / (len(self.classes) - 1)
        y = smooth_labels_with_dist(y, smooth_dist)
...
def get_data_generators(subject_id):
